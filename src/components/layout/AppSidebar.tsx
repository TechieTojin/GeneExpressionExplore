...
const FluvioLoadingScreen = ({ progress }) => (
...
const AppSidebar = () => {
          ...
          <div className="space-y-1">
            ...
            onClick={() => toggleSection(item.title.toLowerCase().replace(/\s+/g, '-'))}>
              ...
              <ChevronDown className={cn(
                ...
                expandedSections[item.title.toLowerCase().replace(/\s+/g, '-')] ? "transform rotate-0" : "transform rotate-180"
              )} />
            </div>
                ...
                >
                  <div className="space-y-1 border-l-2 border-muted pl-2">
                    {item.submenu.map((subItem) => (
                      <SidebarMenuButton key={subItem.title} asChild isActive={isActive(subItem.path)}>
  ...
  return (
    <Sidebar className="border-r bg-sidebar dark:bg-sidebar/95 backdrop-blur-sm">
      {isLoading ? (
        <FluvioLoadingScreen progress={connectionProgress} />
      ) : (
        <>
          ...
          <SidebarContent className="px-4 py-2 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent fluvio-animated-content">
            ...
            {/* Simplified content for authenticated users */}
            {isAuthenticated ? (
              <div>
                {/* User Profile */}
